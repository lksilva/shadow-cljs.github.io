...
https://www.vim.org/scripts/script.php?script_id=4978[Fireplace.vim] é um Vim/Neovim plug-in que fornece uma integraçnao ao Clojure REPL agindo como um cliente https://nrepl.org/[nREPL]. Quando combinado com Shadow-CLJS, também fornece uma integração ClojureScript REPL.
...
Como um cliente https://nrepl.org/[nREPL], https://www.vim.org/scripts/script.php?script_id=4978[Fireplace.vim] depende de https://docs.cider.mx/cider-nrepl/[CIDER-nREPL] (que é um middleware nREPL que provê comum, operações editor-agnostic REPL) portando você precisa incluir essas dependências no <<user-config, ~/.shadow-cljs/config.edn>> ou `shadow-cljs.edn` (como mostra a próxima sub-sessão) Shadow-CLJS vai injetar o obrigatório middleware CIDER-nREPL uma vez que ele vê essa dependência.
